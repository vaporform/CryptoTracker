...
class Widget:
    def __init__(self, parent, colors=[], title="Widget", subtitle="base", middle=None):
        self.parent = parent
        self.colors = colors
        self.frame = tk.Frame(parent, borderwidth=2, relief=tk.RIDGE, padx=10, pady=10)
        self.header = tk.Frame(self.frame)

        # 1. Pack the Button to the RIGHT
        self.button = ttk.Button(self.header, text="Close")
        ...
        if middle is not None:
            c = ttk.Label(self.header, text=middle, font=("Arial", 16, "bold"), anchor="center")
