...
class Ticker(BaseUI.Widget):
    def __init__(self, parent, symbol, colors=[], title="", sub=""):
        ...
        self.symbol = symbol
        self.websocket = CryptoWS(stream=f"{self.symbol}@ticker", on_message=self.on_message, on_error=lambda ws, err: print(f"{self.symbol} error: {err}"), on_close=lambda ws, s, m: print(f"{self.symbol} closed"), on_open=lambda ws: print(f"{self.symbol} connected")
                                  )
